 ...
 * Groq LLM Client
 *
 * Provides integration with Groq's API via OpenAI-compatible endpoint.
 *
...
export class GroqClient extends BaseLLMClient {
  ...
  private handleError(error: unknown): Error {
    logger.warn('Groq API error', { error: error instanceof Error ? error.message : String(error) });
    if (error instanceof OpenAI.APIError) {
      ...
      if (status === 429) {
        ...
        return new LLMRateLimitError(
          `Groq API rate limit exceeded: ${error.message}`,
          retryAfter,
          {
            status,
            type: error.type,
      ...
      if (status === 408 || error.message.includes('timeout')) {
        return new LLMTimeoutError(`Groq API timeout: ${error.message}`, 30000, {
          status,
          type: error.type,
          code: error.code,
        });
 ...
 */
export function createGroqClient(apiKey: string, defaultModel?: string): GroqClient {
