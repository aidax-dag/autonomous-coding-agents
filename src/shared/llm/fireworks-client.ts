 ...
 *
 * Provides integration with Fireworks AI's API via OpenAI-compatible endpoint.
 *
...
const MODEL_CONTEXT_LENGTHS: Record<string, number> = {
  ...
  'accounts/fireworks/models/llama-v3p1-8b-instruct': 128000,
  'accounts/fireworks/models/mixtral-8x22b-instruct': 65536,
};

export class FireworksClient extends BaseLLMClient {
  ...
  /**
   * Handle Fireworks AI API errors
   */
  private handleError(error: unknown): Error {
    logger.warn('Fireworks AI API error', { error: error instanceof Error ? error.message : String(error) });
    if (error instanceof OpenAI.APIError) {
      ...
      if (status === 429) {
        ...
        return new LLMRateLimitError(
          `Fireworks AI API rate limit exceeded: ${error.message}`,
          retryAfter,
          {
            status,
            type: error.type,
      ...
      if (status === 408 || error.message.includes('timeout')) {
        return new LLMTimeoutError(`Fireworks AI API timeout: ${error.message}`, 30000, {
          status,
          type: error.type,
          code: error.code,
        });
      ...
      // General API error
      return new LLMError(
        `Fireworks AI API error: ${error.message}`,
        undefined,
...
/**
 * Factory function to create a Fireworks AI client
 */
export function createFireworksClient(apiKey: string, defaultModel?: string): FireworksClient {
