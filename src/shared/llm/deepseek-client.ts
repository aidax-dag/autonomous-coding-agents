 ...
 * DeepSeek LLM Client
 *
 * Provides integration with DeepSeek's API via OpenAI-compatible endpoint.
 *
...
export class DeepSeekClient extends BaseLLMClient {
  ...
  getMaxContextLength(model?: string): number {
    ...
    return MODEL_CONTEXT_LENGTHS[modelName] || 65536;
  }
  ...
  private handleError(error: unknown): Error {
    logger.warn('DeepSeek API error', { error: error instanceof Error ? error.message : String(error) });
    if (error instanceof OpenAI.APIError) {
      ...
      if (status === 429) {
        ...
        return new LLMRateLimitError(
          `DeepSeek API rate limit exceeded: ${error.message}`,
          retryAfter,
          {
            status,
            type: error.type,
      ...
      if (status === 408 || error.message.includes('timeout')) {
        return new LLMTimeoutError(`DeepSeek API timeout: ${error.message}`, 30000, {
          status,
          type: error.type,
          code: error.code,
        });
 ...
 */
export function createDeepSeekClient(apiKey: string, defaultModel?: string): DeepSeekClient {
