 ...
 *
 * Starts the Coder Agent process for autonomous code implementation.
...
const logger = createAgentLogger('MAIN', 'coder-startup');
...
async function main(): Promise<void> {
  logger.info('Starting Coder Agent...', {
    ...
    pid: process.pid,
    agentId: process.env.AGENT_ID || 'coder-1',
  });
  ...
  try {
    ...
    const config: AgentConfig = {
      id: process.env.AGENT_ID || 'coder-1',
      type: AgentType.CODER,
      name: process.env.AGENT_NAME || 'Coder Agent',
    ...
    const agent = new CoderAgent(config, natsClient);
    await agent.start();

    logger.info('Coder Agent started successfully', {
      agentId: config.id,
    ...
    const shutdown = async (signal: string) => {
      ...
      try {
        ...
        logger.info('Coder Agent stopped successfully');
        process.exit(0);
      } catch (error) {
    ...
    logger.error('Failed to start Coder Agent', { error });
    process.exit(1);
  }
}
