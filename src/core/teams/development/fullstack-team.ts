...
const FULLSTACK_DEVELOPER_ROLE: AgentRole = createRole(
  'Fullstack Developer',
...
const INTEGRATION_ARCHITECT_ROLE: AgentRole = createRole(
...
const DEVOPS_LIAISON_ROLE: AgentRole = createRole(
  'DevOps Liaison',
...
`;
  ...
  protected toResourceName(title: string): string {
    return this.toCamelCase(title.replace(/crud|feature|fullstack/gi, '').trim());
  }

  protected toComponentName(title: string): string {
    return this.toPascalCase(title.replace(/component|ui|page/gi, '').trim());
  }
