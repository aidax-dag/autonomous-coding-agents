...
import { DEFAULT_QA_CONFIG } from './qa-team.config';
import { createQAAgentRoles } from './qa-team.roles';
import { toPascalCase, toKebabCase, extractTestTarget } from './utils';
...
export class QATeam extends BaseTeam {
  ...
  protected getAgentRoles(): AgentRole[] {
    return createQAAgentRoles(this.qaConfig.testFrameworks, this.qaConfig.e2eTool);
  }
  ...
  protected analyzeForUnitTests(task: TaskDocument): TestAnalysis {
    return { ...this.analyzeTestRequirements(task), testType: 'unit' };
  }

  protected analyzeForIntegrationTests(task: TaskDocument): TestAnalysis {
    return { ...this.analyzeTestRequirements(task), testType: 'integration' };
  }

  protected analyzeForE2ETests(task: TaskDocument): TestAnalysis {
    return { ...this.analyzeTestRequirements(task), testType: 'e2e' };
  }

  protected extractTestTarget(task: TaskDocument): string {
    return extractTestTarget(task.title);
  }
